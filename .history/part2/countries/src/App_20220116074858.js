...
const Filter = ({ countries, newFilter }) => {
  const exactSearch = () => {
    // return newFilter.startsWith('[') && newFilter.endsWith(']') // exactSerach alternative
    return newFilter.endsWith('.')
  }

  let countriesToShow = []
  if (exactSearch()) {
    //const exactFilter = newFilter.slice(1, newFilter.length - 1) // exactFilter alternative
    const exactFilter = newFilter.slice(0, newFilter.length - 1)
    countriesToShow = countries.filter(
      (country) =>
        country.name.common.toLowerCase() === exactFilter.toLowerCase()
    )
  } else {
    countriesToShow = countries.filter(
      (country) =>
